...
void ThermalPrinterDisplay::print_image(std::string image, int width) {
  ...
  this->write_array(SET_ROW_SPACING_CMD, sizeof(SET_ROW_SPACING_CMD));
  this->write_byte(0x00);  // Set line spacing to 0 dots
  this->write_byte(0x0A);  // no sure what this does?
  this->write_array(PRINT_BITMAP_CMD, sizeof(PRINT_BITMAP_CMD));
  this->write_byte(0x30);  // hardcoded m of 48 for 203.2 dpi.
  this->write_array(SET_ROW_SPACING_CMD, sizeof(SET_ROW_SPACING_CMD));
  this->write_byte(0x20);  // Reset line spacing to 32 dots
  // 0x1B, 0x33, 0x00,
  // 0x0A, //?
